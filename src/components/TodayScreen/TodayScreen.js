...
export default function TodayScreen() {
  const { userInfo, todayTrack, setTodayTrack, setCounter, counter } =
  ...
  return (
    <Box>
      ...
      {percentageHabits > 0 ? (
        <PercentageDone>
          {percentageHabits}% dos hábitos concluídos
        </PercentageDone>
      ) : (
        <Text>{text}</Text>
      )}
        ...
        <Habits key={index}>
          ...
          <div>
            ...
            >
              ...
              {value.highestSequence > 1 || value.currentSequence === 0
                ? "dias"
                : "dia"}
            </TextRecord>
          </div>
          <CheckBox
            background={value.done}
            onClick={() => checkHabit(value.id, value.done)}
          >
