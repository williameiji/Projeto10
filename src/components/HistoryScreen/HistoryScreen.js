    ...
    <>
      <BoxPopup onClick={() => setControlRender(false)}>
        <PopupContent>
          ...
          {controlRender
            ? renderPopup.habits.map((value, index) => (
                <TextPopup key={index} color={value.done}>
                  {value.name}
                </TextPopup>
              ))
            : null}
...
export default function HistoryScreen() {
  ...
  function tileClassName({ date, view }) {
    if (view === "month") {
      let dayMark = trackHistory.find(
        (dDate) => dDate.day === dayjs(date).format("DD/MM/YYYY")
      );
      if (dayMark) {
  ...
  function click(date) {
    let render = trackHistory.find(
      (track) => track.day === dayjs(date).format("DD/MM/YYYY")
    );
    if (render) {
